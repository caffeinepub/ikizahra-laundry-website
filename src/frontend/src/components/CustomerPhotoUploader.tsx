...
export function CustomerPhotoUploader() {
  ...
  const { data: customBackground, isLoading: _backgroundLoading } =
    useGetPhotoBackgroundImage();
  ...
  return (
    <div className="relative max-w-3xl mx-auto">
      ...
      <div className="absolute inset-0 rounded-3xl overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-br from-white/90 via-sky-50/95 to-blue-50/90" />
      </div>

      {/* Upload Card */}
      <Card className="luxury-card border-sky-200/50 relative z-10 backdrop-blur-sm bg-white/80">
        <CardHeader className="pb-6">
          <CardTitle className="text-2xl text-sky-900 text-center">
            Share Your Photo
          </CardTitle>
          <p className="text-center text-sky-700/80 mt-2">
            Upload a photo to share with us
          </p>
        </CardHeader>
        <CardContent>
            ...
            {!actor && !actorLoading && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-sm text-amber-800 text-center">
                  ⚠️ Backend connection not available. Please wait or refresh the
                  page.
                </p>
              </div>
            )}
