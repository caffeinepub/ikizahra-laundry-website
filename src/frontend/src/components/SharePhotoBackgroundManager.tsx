...
import {
  useGetPhotoBackgroundImage,
  useRemovePhotoBackgroundImage,
  useUploadPhotoBackgroundImage,
} from "../hooks/useQueries";
import {
  type FitMode,
  useSharePhotoBackgroundFit,
} from "../hooks/useSharePhotoBackgroundFit";
import {
...
export function SharePhotoBackgroundManager() {
  ...
  const { data: currentBackground, isLoading: _backgroundLoading } =
    useGetPhotoBackgroundImage();
  ...
  const [processedImage, setProcessedImage] =
    useState<ProcessedImageInfo | null>(null);
  ...
  const processImage = async (
    ...
    res: ResolutionChoice,
  ) => {
    setIsProcessing(true);
    try {
      const processed = await processImageWithAspectRatio(
        file,
        ratio,
        res,
        0.85,
  ...
  const handleResolutionChange = async (value: string) => {
    const res =
      value === "original"
        ? "original"
        : (Number.parseInt(value) as ResolutionChoice);
    setResolution(res);
    if (selectedFile) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success("Background image uploaded successfully!", {
        description:
          "The Share Your Photo section will now use your custom image.",
      });
  ...
  const isRemoveDisabled =
    !actor || actorLoading || !currentBackground || removeMutation.isPending;

  return (
    <Card className="luxury-card border-sky-200/50">
      <CardHeader>
        <CardTitle className="text-xl text-sky-900">
          Share Your Photo - Background Image
        </CardTitle>
      ...
      <CardContent className="space-y-6">
        ...
        {currentBackground && (
          <div className="space-y-2">
            <Label className="text-sky-900 font-medium">
              Current Background
            </Label>
          ...
          <div className="bg-sky-50/50 rounded-xl p-4 border border-sky-200">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm text-sky-700">
              ...
              <div>
                ...
                {resolution === "original" ? "Original" : `${resolution}px`}
              </div>
            </div>
          </div>
        )}
